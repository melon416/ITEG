# Drew comment #!/opt/fedex/symphony/current/python/bin/python2.7
"""
COPYRIGHT
All Rights Reserved.
...
import cx_Oracle, os, sys, getpass, re, tffolder, logging, tfutil
...
def init():
  ...
  parser.add_option("--project",
                    ...
                    type='string',
                    default='',
                    help='The Symphony project name to be exported.')
  parser.add_option('--DBUser',
                    dest='dbuser',
                    action='store',
                    type='string',
                    default='',
                    help='The name to use when connecting to the database.')
  parser.add_option('--targetdir',
                    dest='targetdir',
                    action='store',
                    type='string',
                    default='/tmp',
                    help='The base path for the target directory.')
  parser.add_option('--incident',
                    dest='incident',
                    action='store',
                    type='string',
                    default='',
                    help='The PDSM Incident being worked.')
  parser.add_option('--type',
                    dest='type',
                    action='store',
                    type='string',
                    default='zip',
                    help='The type of zip to produce either "zip" or "tgz" - defaults to "zip".')
  parser.add_option('--filestore',
                    dest='filestore',
                    action='store',
                    type='string',
                    default='/var/fedex/teamforge/filestorage',
                    help='The path to the TeamForge filestore direcotry.  DEFAULTS TO:  /var/fedex/teamforge/filestorage')
  parser.add_option('--DBSchema',
                    dest='dbschema',
                    action='store',
                    type='string',
                    default='SYM_SYMPHONY_SVC_PRD',
                    help='DEFAULTS TO:  SYM_SYMPHONY_SVC_PRD')
  
  (options, args) = parser.parse_args()

  if (0 == len(options.project)) or (0 == len(options.dbuser)) or (0 == len(options.incident)):
    print("all of these:   --project and --DBUser are required and --incident")
    parser.print_help()
    sys.exit(1)
  if (options.type not in set(['zip','tgz'])):
  ...
  try:
    os.mkdir(options.targetdir + '/' + options.project)
  except:
...
def getProjectNumber(connection, projectName):
  ...
  csr.execute("select PROJECT_NBR, PROJECT_NM  from sym_symphony_schema.TOOL_PROJECT where TOOL_CD = 'SF' and PROJECT_NM " +  operator + " '" + projectName + "'")
  res = csr.fetchall()
